...
session_start();
	require_once('dbconnect.php');
			...
			$team_name =  trim(mysqli_real_escape_string($con,$_POST['prath_team_name']));
			$team_email =  trim(mysqli_real_escape_string($con,$_POST['prath_team_email']));
			$team_pass =  trim(mysqli_real_escape_string($con,sha1("$_POST[prath_team_pass]")));
			$team_contact = trim(mysqli_real_escape_string($con,$_POST['prath_team_contact']));
			$team_address =  trim(mysqli_real_escape_string($con,$_POST['prath_team_address']));
			$team_size =  trim(mysqli_real_escape_string($con,$_POST['prath_team_size']));
			...
			if(empty($_POST['prath_team_pass'])==true){
				echo "Please enter a password";
				exit(0);
			}
			if($_POST['prath_team_pass']!= $_POST['prath_team_repass']){
	...
	$rep = mysqli_query($con,"INSERT INTO `team_info`(`team_name`, `team_contact_email`, `event_type`, `team_contact`, `team_address`, `team_size`, `team_pass`) VALUES ('$team_name','$team_email','PRS','$team_contact','$team_address','$team_size','$team_pass') ");

	
	$id = mysqli_insert_id($con);
	foreach ($_POST['prath_mem_name'] as $index => $value) {
		# code...
				$sem = mysqli_real_escape_string($con,trim($_POST['prath_mem_sem'][$index]));
				$gen = mysqli_real_escape_string($con,trim($_POST['prath_mem_gen'][$index]));
				$col = mysqli_real_escape_string($con,trim($_POST['prath_mem_col'][$index]));
